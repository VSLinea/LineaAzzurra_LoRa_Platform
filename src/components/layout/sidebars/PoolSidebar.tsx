...
const getNavigation = (role: string): NavigationItem[] => {
  const baseNavigation = [
    { name: 'Pool Overview', href: '/dashboard', icon: LayoutDashboard },
  ...
  // Pool Manager gets access to reports
  ...
  // Technician gets access to maintenance tasks
  if (role === 'TECHNICIAN') {
    return [
      ...baseNavigation,
  ...
  // Pool Viewer gets basic navigation only
...
export default function PoolSidebar() {
  ...
  const navigation = getNavigation(session?.user?.role || 'POOL_VIEWER')
      ...
      <div className="p-4">
        ...
        <div className="px-3 mb-2">
          <h2 className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">
            {session?.user?.role === 'POOL_MANAGER' ? 'Pool Manager' :
             session?.user?.role === 'TECHNICIAN' ? 'Pool Technician' :
             'Pool Monitor'}
          </h2>
        </div>
